...
/**
 * Provides a friendly event driven API to luxd in Node.js. Manages starting and
 * stopping luxd as a child process for application support, as well as connecting
 * to multiple luxd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Lux(options) {
...
util.inherits(Lux, Service);

Lux.dependencies = [];

Lux.DEFAULT_MAX_TXIDS = 1000;
Lux.DEFAULT_MAX_HISTORY = 50;
Lux.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Lux.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Lux.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Lux.DEFAULT_SPAWN_RESTART_TIME = 5000;
Lux.DEFAULT_SPAWN_STOP_TIME = 10000;
Lux.DEFAULT_TRY_ALL_INTERVAL = 1000;
Lux.DEFAULT_REINDEX_INTERVAL = 10000;
Lux.DEFAULT_START_RETRY_INTERVAL = 5000;
Lux.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Lux.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Lux.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'lux',
  rpcpassword: 'luxpassword',
  uacomment: 'bitcore'
};

Lux.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Lux.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Lux.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Lux.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Lux.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Lux.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Lux.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Lux.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Lux.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Lux.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Lux.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Lux.prototype._initCaches = function() {
...
Lux.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Lux.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Lux.prototype.getPublishEvents = function() {
  return [
    {
...
Lux.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'luxd/' + name, 'total:', this.subscriptions[name].length);
};

Lux.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'luxd/' + name, 'total:', this.subscriptions[name].length);
};


Lux.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Lux.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Lux.prototype.unsubscribeBalanceAll = function(emitter) {
...
Lux.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'luxd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Lux.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'luxd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Lux.prototype.unsubscribeAddressAll = function(emitter) {
...
Lux.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Lux.DEFAULT_CONFIG_SETTINGS;
...
Lux.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Lux.prototype._expandRelativeDatadir = function() {
...
Lux.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in luxd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in luxd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in luxd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './lux.conf');

  log.info('Using lux config file:', configPath);
...
Lux.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of luxcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of luxcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of luxcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to luxd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from luxd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from luxd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in lux.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that luxd is undergoing a reindex. ' +
             ...
             'of luxcore-node services will start.');
    node._reindex = true;
  }
};

Lux.prototype._resetCaches = function() {
...
Lux.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Lux.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Lux.prototype._initChain = function(callback) {
  var self = this;
...
Lux.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 9888;
  }
  return networkOptions;
};

Lux.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/lux.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/lux.conf';
    }
  }
  return networkPath;
};

Lux.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Lux.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('luxd/hashblock', message.toString('hex'));
    }
  }

};

Lux.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Lux.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Lux Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Lux.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Lux.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Lux.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Lux.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('luxd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Lux.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('luxd/rawtransaction', message.toString('hex'));
    }
...
Lux.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Lux.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Lux.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Lux.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Lux.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Lux Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Lux.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Lux.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Lux.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/luxd.pid';
...
Lux.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Lux process unexpectedly exited with code:', code);
        log.warn('Restarting lux child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Lux process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn luxd.'));
      }
...
Lux.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to luxd.'));
    }
 ...
 */
Lux.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Lux configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Lux.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Lux.prototype.syncPercentage = function(callback) {
  var self = this;
...
Lux.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, luxcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Lux.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Lux.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Lux.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Lux.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in luxcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Lux.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Lux.prototype._getConfirmationsDetail = function(transaction) {
...
Lux.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Lux.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Lux.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A luxcoin transaction id
 * @param {Function} callback
 */
Lux.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Lux.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Lux.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Lux.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Lux.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Lux.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Lux.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Lux.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Lux.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Lux.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Luxcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Lux.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getInfo = function(callback) {
...
Lux.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('luxd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('luxd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Lux.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Lux.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Lux.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Lux.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Lux.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Lux.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Lux.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Lux.prototype.getStakingInfo = function(callback) {
    var self = this,
